 ...
 * CONTRACT:
 *  - GET /api/project.php?token=... → { ok:true, mode:"production"|"fallback", productConfig }
 * SECURITY:
 *  - no-store headers
...
if ($token === '' || !isset($map[$token]) || !is_array($map[$token])) {
  // FALLBACK: bezpiecznie — demo (bez wysyłki)
  respond([
    'ok' => true,
    'mode' => 'fallback',
    'productConfig' => [
...
/* === KONIEC PLIKU — api/project.php | FILE_VERSION: 2026-02-10-02 === */
